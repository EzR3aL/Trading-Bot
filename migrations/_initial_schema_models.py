"""
Frozen ORM schema for Alembic revision 001 (#350).

This file is a **verbatim copy** of ``src/models/database.py`` at commit
``0551c2c`` (2026-02-19), the commit that introduced the Alembic migration
framework. It is NOT imported by any runtime code — only by
``migrations/versions/001_initial_schema.py`` so that
``Base.metadata.create_all()`` in revision 001 creates the schema as it
existed when 001 was authored, NOT the current live ORM schema.

Why this file exists (#350): the original 001 imported
``src.models.database.Base`` directly, which meant a fresh
``alembic upgrade head`` on an empty DB would (a) create the fully-evolved
current schema via ``create_all()`` in 001, then (b) fail at the first
post-001 ``op.add_column()`` because that column already existed
(``DuplicateColumnError``). Freezing the schema here breaks that loop:
001 only creates the 2026-02-19 schema, and migrations 002..N apply
their diffs cleanly.

**Do not edit this file.** If the live ORM changes, that change belongs
in a new Alembic migration — not here. This file should be modified
only to fix bugs that existed at commit ``0551c2c``.
"""
# ruff: noqa: F401 — preserved verbatim from commit 0551c2c; `datetime`
# and `Optional` were imported for type hints in the original ORM file.
