...
### Fixed
- **BingX + Hyperliquid Close-Readback robustness (#224, Epic #188 Follow-Up)**: **BingX** — `get_close_reason_from_history` fehlten `endTime`- und `limit`-Params auf `allOrders`; BingX limitierte stillschweigend auf Default-7d-Fenster und max. 500 Rows, sehr aktive Accounts verloren ältere Fills. Fix: `endTime` + `limit=1000` ergänzt, `until_ts_ms` durchs Interface geführt für Backfill-Nutzung. **Hyperliquid** — `user_fills` hatte keinen try/except um die SDK-Calls (Anti-Pattern C): ein transientes SDK-Fehler bubbelte in `RiskStateManager.classify_close()` und killte die Klassifizierung statt graceful auf Heuristik zurückzufallen. Außerdem markierte der Adapter alle `isTpsl`-Fills blind als `pos_profit` — echte SL-Trigger wurden deshalb als TAKE_PROFIT_NATIVE fehlklassifiziert wenn `trade.sl_order_id` nicht im DB stand. Fix: `user_fills` ist jetzt in try/except isoliert (Return `None` bei Fehler → Heuristik-Fallback), isTpsl-Fills liefern neuen Sentinel `tpsl_ambiguous`. **RiskStateManager** — neuer Price-Crossover-Disambiguator in `_classify_from_snapshot`: wenn `plan_type` `tpsl_ambiguous`/`pos_profit`/`pos_loss` ist UND der `order_id`-Match gescheitert ist, vergleicht er `fill_price` gegen `trade.take_profit`/`trade.stop_loss` + `trade.side` und liefert die korrekte TP/SL-Klassifizierung. Fällt sauber auf `EXTERNAL_CLOSE_UNKNOWN` zurück wenn weder TP noch SL gesetzt sind. Threading von `until_ts_ms` durch beide Adapter abgeschlossen (macht den Backfill auf BingX/HL korrekt).
