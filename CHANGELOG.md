...
### Changed
- **ARCH-H1 Phase 1 PR-6 — BotWorker finalize auf reine Composition (#285)**: Abschluss der Mixin→Composition-Migration aus Phase 1 (#274/#277/#279/#281/#72). `class BotWorker(TradeExecutorMixin, PositionMonitorMixin, TradeCloserMixin, HyperliquidGatesMixin, NotificationsMixin)` wurde zu `class BotWorker:` — keine Mehrfachvererbung mehr, `BotWorker.__mro__` enthält nur noch `(BotWorker, object)`. Die fünf Komponenten (`Notifier`, `HyperliquidGates`, `TradeCloser`, `PositionMonitor`, `TradeExecutor`) werden direkt in `BotWorker.__init__` über Getter-Callables aufgebaut (`config_getter=lambda: self._config` respektiert den deferred Lifecycle bei dem `_config`/`_risk_manager`/`_strategy`/`_risk_state_manager` erst nach `initialize()` attached werden). Alle ehemaligen Mixin-Methoden sind durch explizite Forwarder ersetzt: `_send_notification`, `_get_discord_notifier`, `_get_notifiers` (Notifier), `_check_referral_gate`, `_check_builder_approval`, `_check_wallet_gate`, `_check_affiliate_uid_gate` (HyperliquidGates), `_close_and_record_trade` (TradeCloser), `_monitor_positions_safe`/`_monitor_positions`/`_check_position`/`_try_place_native_trailing_stop`/`_confirm_position_closed`/`_check_pnl_alert`/`_classify_close_heuristic`/`_handle_closed_position` (PositionMonitor), `_execute_trade`/`_resolve_pending_trade`/`_notify_trade_failure`/`get_open_trades_count`/`get_open_trades_for_bot`/`execute_trade`/`close_trade_by_strategy` (TradeExecutor). Worker-State-Mutationen laufen über zwei Callbacks an `TradeExecutor`: `_on_trade_opened` → `trades_today += 1`, `_on_fatal_trade_error` → `BotStatus.ERROR` + `error_message`. **Shim-Dateien gelöscht:** `src/bot/notifications.py`, `src/bot/hyperliquid_gates.py`, `src/bot/trade_closer.py` (keine Live-Referenzen). **Shim-Dateien erhalten:** `src/bot/position_monitor.py` und `src/bot/trade_executor.py` bleiben als Thin-Proxy-Mixins + Konstanten-Re-Exports, weil `tests/unit/test_production_hardening.py`, `tests/unit/test_strategy_exit_and_optional_tpsl.py`, `tests/unit/bot/test_tpsl_passthrough.py` und `tests/unit/bot/test_position_monitor_classifier.py` die Mixins direkt als Test-Harnesses instanziieren. Zero-Behavior-Change — sämtliche öffentlichen Methoden und Attribute (`initialize`, `start`, `stop`, `status`, `config`, `error_message`, `get_status_dict`, `graceful_stop`, `_shutting_down`) bleiben unverändert; Orchestrator- und API-Router-Pfade unberührt. Full Unit-Suite grün nach dem Finalize. Supersedes #276/#278/#280/#282/#284.
...
### Changed
- **ARCH-H1 Phase 1 PR-2 — `HyperliquidGatesMixin` → `HyperliquidGates`-Komponente (#277)**: Zweiter Composition-Schritt nach PR-1 (Notifier). Neue Datei `src/bot/components/hyperliquid_gates.py` enthält die `HyperliquidGates`-Klasse (4 Gate-Checks: `check_referral`, `check_builder_approval`, `check_wallet`, `check_affiliate_uid`) plus `GateResult`-Value-Object (`@dataclass(frozen=True)` mit `ok: bool` + `error_message: Optional[str]`, plus `.passed()` / `.blocked(msg)` Factories). Zero-Callsite-Change-Strategie: `src/bot/hyperliquid_gates.py::HyperliquidGatesMixin` wird auf ~45 LOC thin proxy reduziert, der weiterhin `_check_referral_gate`/`_check_builder_approval`/`_check_wallet_gate`/`_check_affiliate_uid_gate` exponiert — intern delegiert jede Methode an `self._hl_gates.check_*(...)` und übersetzt das `GateResult` zurück in den Legacy-Seiteneffekt-Kontrakt (`self.error_message = ...` + `self.status = "error"` bei Failure). Die 3 bestehenden Call-Sites in `src/bot/bot_worker.py:308-316` bleiben komplett unverändert. `BotWorker.__init__` wired die Komponente via `self._hl_gates = HyperliquidGates(bot_config_id, lambda: self._config)` — der Config-Getter ist essentiell, weil `_config` erst in `start()` geladen wird, nicht in `__init__`. Value-Object-Ansatz (statt Callbacks) entkoppelt die Gate-Logik sauber von der BotWorker-State-Mutation: Tests mocken jetzt nur DB/Client, nicht den Worker selbst. 22 neue Unit-Tests in `tests/unit/bot/components/test_hyperliquid_gates.py` decken jede Gate-Methode ab (happy path, DB-fast-path, blocked-with-message, exception-handling; plus 2 Smoke-Tests für `GateResult`-Factories). Alle 111 bestehenden BotWorker-Tests grün — keine Regression. PR-2 ist unabhängig von PR-1 — beide können in beliebiger Reihenfolge gemerged werden. Nächster Schritt: PR-3 (TradeCloser-Extraktion).
