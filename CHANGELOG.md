...
### Fixed
- **Bitget Close-Readback Query komplett broken (#221, Epic #188 Hotfix)**: `_fetch_bitget_plan_close` hatte drei Bugs die den Call seit #191 jedes Mal mit "Parameter verification failed" crashen ließen — (a) `planType`-Param fehlte obwohl Bitget v2 ihn required macht, (b) `endTime` fehlte, (c) der Status-Filter prüfte `planStatus == "triggered"` aber Bitget liefert `executed` zurück. Die `get_close_reason_from_history`-Methode fiel deshalb auf jedem Bitget-Trade in den Heuristik-Fallback — der Exchange-Readback-Classifier aus #193 war effektiv toter Code auf Bitget. Fix: `planType=profit_loss` (Umbrella für pos_profit/pos_loss/track_plan im Response), `endTime=now`, Status-Filter akzeptiert `executed` und `triggered`. `executeOrderId` wird jetzt dem Plan-`orderId` vorgezogen — das ist die ID die auch in `orders-history` als Fill auftaucht und mit `TradeRecord.sl_order_id/tp_order_id/trailing_order_id` matcht. Zusätzlich `_fetch_bitget_manual_close` verbessert: liest jetzt `orderSource` und mappt `pos_loss_market`/`pos_profit_market`/`track_plan_market` auf die korrekten plan_type-Keys statt alles als `manual` zu flaggen — so klassifiziert auch der Manual-Close-Fallback TP/SL-Trigger richtig wenn plan-history die Row schon GCed hat. `get_close_reason_from_history` isoliert beide Probes in eigenen try/except so dass ein Fehler auf einer Quelle die andere nicht unterdrückt. Verhindert endgültig Anti-Pattern B auf Bitget.
