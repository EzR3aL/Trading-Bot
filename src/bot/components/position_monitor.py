"""Position monitoring (ARCH-H1 Phase 1 PR-4, #281).

Extracted from ``PositionMonitorMixin`` so the exchange-driven position
reconciliation, trailing-stop placement, strategy-exit handling, glitch
detection, and PnL alerting live in a composition-owned component that
is independently testable. The mixin is kept as a thin proxy until the
Phase 1 finalize PR removes all mixin shims.

The component owns its own per-instance state (trailing-stop backoff,
glitch counter, PnL alert cache). Dependencies that are attached to
``BotWorker`` at different lifecycle points are injected via getters/
callables so the component tolerates the deferred lifecycle.
"""
...
class PositionMonitor:
    """Owns the per-bot position monitoring loop and its side effects.

    Dependencies are supplied as callables so the component tolerates the
    deferred lifecycle of ``BotWorker`` (``_config``, ``_strategy``, and
    ``_risk_state_manager`` are attached after ``__init__`` has run).
    """
    ...
    def __init__(
        ...
        strategy_getter: Callable[[], Optional[Any]],
        risk_state_manager_getter: Callable[[], Optional[RiskStateManager]],
        client_factory: Callable[[bool], Optional[ExchangeClient]],
        ...
        self._get_strategy = strategy_getter
        self._get_risk_state_manager = risk_state_manager_getter
        self._get_client = client_factory
        self._close_trade = close_trade
        self._send_notification = notification_sender

        # Per-instance monitor state — lives with the component so each bot
        # has its own trackers rather than module-level globals.
        ...
        self._pnl_alert_parsed: Optional[dict] = None

    async def monitor_safe(self) -> None:
        """Wrapper with error handling for the scheduled monitor tick."""
        try:
            await self.monitor()
        except Exception as e:
            logger.error(f"[Bot:{self._bot_config_id}] Monitor error: {e}")
        ...
        async with get_session() as session:
            ...
            result = await session.execute(
                select(TradeRecord).where(
                    TradeRecord.bot_config_id == self._bot_config_id,
            ...
            open_glitch_keys = {f"{self._bot_config_id}:{t.symbol}" for t in open_trades}
            ...
            if config and config.pnl_alert_settings:
                try:
                    parsed = json.loads(config.pnl_alert_settings)
            ...
            for trade in open_trades:
                await self.check_position(trade, session)

    async def check_position(self, trade: TradeRecord, session) -> None:
        ...
        try:
            ...
            if not position:
                ...
                confirmed = await self.confirm_position_closed(trade, client)
                if confirmed:
                    await self.handle_closed_position(trade, client, session)
                return
            ...
            if hasattr(position, "side") and trade.side != position.side:
                if trade.side not in ("long", "short"):
                    logger.info(
                        ...
                        self._bot_config_id, trade.id, trade.side, position.side,
                    )
            ...
            if current_price and trade.entry_price:
                await self.check_pnl_alert(trade, current_price)
                ...
                try:
                    ...
                    if exch_has_trail != bool(trade.native_trailing_stop):
                        logger.info(
                            ...
                            "DB=%s exchange=%s",
                            self._bot_config_id, trade.symbol, trade.side,
                    ...
                    logger.debug(
                        ...
                        self._bot_config_id, trade.symbol, e,
                    )

            # Skip the monitor's auto-placement when another actor is
            # actively working on the trailing leg. See _TRAILING_SKIP_STATES.
            ...
            if (
                ...
                if should_retry:
                    await self.try_place_native_trailing_stop(
            ...
            has_trailing = (
                ...
                or (
                    strategy
                    and hasattr(strategy, "_p")
                    and strategy._p.get("trailing_stop_enabled")
                )
            )
            if has_exchange_tpsl and not has_trailing:
                logger.debug(
                    "[Bot:%s] Skipping should_exit for %s — exchange TP/SL active (TP=%s SL=%s)",
                    self._bot_config_id, trade.symbol, trade.take_profit, trade.stop_loss,
                )
                return
            ...
            if strategy and hasattr(strategy, "should_exit"):
                try:
                    ...
                    if trade.metrics_snapshot:
                        try:
                            metrics_at_entry = (
                                json.loads(trade.metrics_snapshot)
                                if isinstance(trade.metrics_snapshot, str)
                                else trade.metrics_snapshot
                            )
                        except (json.JSONDecodeError, TypeError):
                            pass

                    should_close, exit_reason = await strategy.should_exit(
                        symbol=trade.symbol,
                        side=trade.side,
                    ...
                    if should_close:
                        logger.info(
                            ...
                            self._bot_config_id, trade.symbol, trade.side, exit_reason,
                        )
                        # Mark the strategy-exit BEFORE the close call so a
                        # late-arriving handle_closed_position on the next
                        # cycle still sees the signal in the manager cache.
                        rsm = self._get_risk_state_manager()
                        if rsm is not None:
                            try:
                                rsm.note_strategy_exit(trade.id)
                            except Exception as note_err:
                                logger.debug(
                                    "[Bot:%s] note_strategy_exit failed for #%s: %s",
                                    self._bot_config_id, trade.id, note_err,
                                )
                        config = self._get_config()
                        try:
                            margin_mode = getattr(config, "margin_mode", "cross")
                            close_order = await client.close_position(
                                trade.symbol, trade.side, margin_mode=margin_mode,
                            )
                        except Exception as close_err:
                            logger.error(
                                "[Bot:%s] Failed to close position: %s",
                                self._bot_config_id, close_err,
                            )
                            return
                        ...
                        if close_order is None:
                            ...
                            logger.info(
                                ...
                                "deferring to handle_closed_position on next cycle",
                                self._bot_config_id, trade.symbol,
                            )
                            return
                        ...
                        strategy_reason = "[%s] %s" % (
                            getattr(config, "name", "?"), exit_reason,
                        )
                    ...
                    logger.error(
                        "[Bot:%s] Strategy exit check error: %s",
                        self._bot_config_id, e,
                    )

        except Exception as e:
            logger.error(
                "[Bot:%s] Position check error: %s", self._bot_config_id, e,
            )

    async def try_place_native_trailing_stop(
        ...
        try:
            # Drift is already reconciled by the outer probe in check_position
    ...
    async def confirm_position_closed(
        ...
        Guards against transient API errors / empty responses that would
        otherwise cause a live position to be prematurely marked as closed.
        """
        log_prefix = f"[Bot:{self._bot_config_id}]"
        glitch_key = f"{self._bot_config_id}:{trade.symbol}"

        # Track whether all retries threw exceptions (Bug 1 fix).
        any_exception = False
        for attempt in range(1, _POSITION_GONE_THRESHOLD + 1):
    ...
    async def check_pnl_alert(self, trade: TradeRecord, current_price: float) -> None:
    ...
    @staticmethod
    def classify_close_heuristic(trade: TradeRecord, exit_price: float) -> str:
    ...
    async def handle_closed_position(
        ...
        try:
            ...
            # defer to its exchange-readback classifier; otherwise fall back
            # to the legacy heuristic so behaviour is unchanged when the flag
            # is off.
            rsm = self._get_risk_state_manager()
            if rsm is not None:
                try:
                    exit_reason = await rsm.classify_close(
                    ...
                    exit_reason = self.classify_close_heuristic(trade, exit_price)
            else:
                exit_reason = self.classify_close_heuristic(trade, exit_price)
            ...
            # Builder-fee revenue (Hyperliquid only).
            builder_fee = 0
            try:
            ...
            config = self._get_config()
            strategy_reason = f"[{getattr(config, 'name', '?')}]"
            await self._close_trade(
                trade,
                exit_price,
                exit_reason,
