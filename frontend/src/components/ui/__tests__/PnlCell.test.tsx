...
describe('PnlCell', () => {
  it('should display formatted positive PnL', () => {
    render(<PnlCell pnl={150.75} fees={5} fundingPaid={2} />)

    expect(screen.getByText('+$150.75')).toBeInTheDocument()
  })

  it('should display formatted negative PnL', () => {
    render(<PnlCell pnl={-42.30} fees={3} fundingPaid={1} />)

    // formatPnl: prefix is '' for negative, so output is "$-42.30"
  ...
  it('should display zero PnL with plus prefix', () => {
  ...
  it('should apply loss color class for negative PnL', () => {
    ...
    // formatPnl: prefix is '' for negative, so output is "$-50.00"
    const span = screen.getByText('$-50.00')
