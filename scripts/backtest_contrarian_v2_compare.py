"""
A/B Backtest: Contrarian Pulse v2 (optimized) vs baseline parameters.

Tests the impact of:
1. EMA bypass for ultra-extreme F&G
2. RSI divergence (replaces CVD)
3. EMA200 proximity (replaces OI>0 free pass)
4. Graduated confidence scoring
"""
...
# v2 optimized defaults (already in the strategy code) vs overrides
PARAM_SETS = {
    # v2 with all improvements (current defaults)
    "v2 default": {},
    "v2 wide F&G 35/65": {"fg_extreme_fear": 35, "fg_extreme_greed": 65},
    "v2 1-confirm": {"min_confirmations": 1},
    "v2 3-confirms": {"min_confirmations": 3},
    # v2 with ultra bypass disabled (simulate v1 behavior)
    "v2 no-bypass": {"fg_ultra_fear": 0, "fg_ultra_greed": 100},
    # v2 with different TP/SL
    ...
    # v2 with aggressive parameters (best-case)
    "v2 aggressive": {
        "fg_extreme_fear": 35,
        "fg_extreme_greed": 65,
        "fg_ultra_fear": 25,
...
async def main():
    print("=" * 110)
    print("  CONTRARIAN PULSE v2 A/B BACKTEST")
    ...
    # Results table
    print(f"\n{'=' * 120}")
    print(f"CONTRARIAN PULSE v2 ERGEBNISSE")
    print(f"{'=' * 120}")
    ...
    print(f"\nNote: Mock data | Fees: Bitget Standard")
